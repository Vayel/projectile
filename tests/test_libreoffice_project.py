...
def test_open_document(emulate_project_folder):
    p = LibreOfficeProject('1', PROJECT_FOLDER)
    p.create_folder()
...
def test_get_ods_name_with_ods(emulate_project_folder):
    p = LibreOfficeProject('1', PROJECT_FOLDER)
    p.create_folder()
    fill_folder(str(p.get_document_path()))
...
def test_get_ods_name_without_ods(emulate_project_folder):
    p = LibreOfficeProject('1', PROJECT_FOLDER)
    p.create_folder()
...
def test_setup(emulate_project_folder):
    name = 'Mon projet'
